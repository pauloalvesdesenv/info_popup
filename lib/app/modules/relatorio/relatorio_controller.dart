...
import 'package:aco_plus/app/modules/relatorio/ui/ordem/relatorio_ordem_pdf_page.dart';
...
class PedidoController {
  ...
  void onCreateRelatorioPedido() {
    final model = RelatorioPedidoModel(
      ...
      FirestoreClient.pedidos.data
          .where((e) => pedidoViewModel.status == RelatorioPedidoStatus.produzindo
  ...
  void onCreateRelatorioOrdem() {
    ...
    for (final ordem in ordens) {
      ordem.produtos =
          ordem.produtos.where((e) => _whereProductStatus(e, ordemViewModel.status!)).toList();
    }
    ...
    final model = RelatorioOrdemModel(
  ...
  bool _whereProductStatus(PedidoProdutoModel produto, RelatorioOrdemStatus status) {
    final productStatus = produto.statusess.last.status;
    switch (status) {
      case RelatorioOrdemStatus.AGUARDANDO_PRODUCAO:
        return [PedidoProdutoStatus.separado, PedidoProdutoStatus.aguardandoProducao]
  ...
  Future<void> onExportRelatorioOrdemPDF() async {
    ...
    pdf.addPage(pw.Page(
        pageFormat: PdfPageFormat.a4,
        build: (pw.Context context) =>
            RelatorioOrdemPdfPage(ordemViewModel.relatorio!).build(imageBytes)));
